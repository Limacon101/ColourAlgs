...
public class HSLColor
{
    ...
    public static float[] fromRGB(Color color,float[] dest)
    {
       ...
       /* System.out.println(min);
        ...
        if (lum < 0.5)
            ...
            saturation = (max-min)/(2-max-max);

        saturation = (float)Math.round(saturation); */
    ...
    public static int[] toRGB(float h, float s, float l, float alpha)
    {
        ...
        int[] lis = new int[] {0,0,0};
        lis[0] = r;
        lis[1] = g;
        lis[2] = b;

        return lis;
        // System.out.println((alphaInt << 24) + ( r << 16 ) + ( g << 8 ) + (b));
